...
std::map<Stub, Avoid::ConnEnd>
PlaceBoxStubConnEnds(const ActivityDiagram &LayedOutDiagram) {
  ...
  for (const ActivityBox &SelectedBox : LayedOutDiagram.Boxes) {
    ...
    for (Stub SelectedStub : SelectedBox.OutputStubs) {
      ...
      StubColumn =
          SelectedOutputStub.Position.Column + SelectedOutputStub.Length;
    ...
    for (Stub SelectedStub : SelectedBox.MechanismStubs) {
      ...
      StubRow =
          SelectedMechanismStub.Position.Row + SelectedMechanismStub.Length;
      AvoidX = StubColumn;
      AvoidY = LayedOutDiagram.Height - StubRow;
      ConnectionEnd = Avoid::ConnEnd(Avoid::Point(AvoidX, AvoidY));
      BoxStubsMap.insert({SelectedStub, ConnectionEnd});
...
std::map<Stub, Avoid::ConnEnd>
PlaceBoundaryStubConnEnds(const ActivityDiagram &LayedOutDiagram) {
  ...
  for (const Stub BoundaryStub : LayedOutDiagram.ControlBoundaryStubs) {
    ...
    const ControlStub &BoundaryControlStub =
        std::get<ControlStub>(BoundaryStub);
    Avoid::ConnEnd ConnectionEnd;
...
void PlaceObstacles(const ActivityDiagram &LayedoutDiagram,
  ...
  for (uint32_t BoxIndex = 0u; BoxIndex < NumBoxes; BoxIndex++) {
    ...
    BoxBottomRight.Column =
        SelectedBox.Center.Column + (SelectedBox.Width / 2u);
...
Avoid::Router *ConstructRouter(std::map<Stub, Avoid::ConnEnd> &BoxStubsMap,
  ...
  for (uint32_t RectangleIndex = 0u; RectangleIndex < NumRects;
       RectangleIndex++) {
  ...
  for (const std::pair<Stub, Avoid::ConnEnd> &BoundaryStubPair :
       BoundaryStubsMap) {
      ...
      if (std::holds_alternative<InputStub>(BoundaryStub)) {
        ...
        if (std::holds_alternative<InputStub>(BoxStub)) {
          ...
          if (BoxInputStub.Name == BoundaryInputStub.Name) {
            ...
            NewConn = new Avoid::ConnRef(
                ConstructedRouter, BoundaryStubPair.second, BoxStubPair.second);
          } else {
            for (const StubSource &BoxInputSource : BoxInputStub.Sources) {
              if (BoxInputSource.StubName == BoundaryInputStub.Name) {
                Avoid::ConnRef *NewConn;

                NewConn = new Avoid::ConnRef(ConstructedRouter,
                                             BoundaryStubPair.second,
                                             BoxStubPair.second);
              }
            }
          }
        ...
        if (std::holds_alternative<MechanismStub>(BoxStub)) {
          const MechanismStub &BoxMechanismStub =
              std::get<MechanismStub>(BoxStub);
          ...
          if (BoxMechanismStub.Name == BoundaryMechanismStub.Name) {
            ...
            for (const StubSource &BoxMechanismSource :
                 BoxMechanismStub.Sources) {
  ...
  for (const std::pair<Stub, Avoid::ConnEnd> &BoxStubPair : BoxStubsMap) {
    ...
    for (const std::pair<Stub, Avoid::ConnEnd> &OtherBoxStubPair :
         BoxStubsMap) {
      ...
      if (std::holds_alternative<OutputStub>(FirstStub)) {
        ...
        if (std::holds_alternative<InputStub>(OtherStub)) {
          ...
          if (FirstOutputStub.Name == OtherInputStub.Name) {
            ...
            NewRef = new Avoid::ConnRef(ConstructedRouter, BoxStubPair.second,
                                        OtherBoxStubPair.second);
          } else {
            for (const StubSource &Source : OtherInputStub.Sources) {
              if (Source.StubName == FirstOutputStub.Name) {
                ...
                NewRef =
                    new Avoid::ConnRef(ConstructedRouter, BoxStubPair.second,
                                       OtherBoxStubPair.second);
              }
            }
          }
        } else if (std::holds_alternative<ControlStub>(OtherStub)) {
          const ControlStub &OtherControlStub =
              std::get<ControlStub>(OtherStub);
          ...
          const MechanismStub &OtherMechanismStub =
              std::get<MechanismStub>(OtherStub);
    ...
    for (const std::pair<Stub, Avoid::ConnEnd> &BoundaryBoxStubPair :
         BoundaryStubsMap) {
      ...
      if (std::holds_alternative<OutputStub>(FirstStub)) {
        ...
        if (std::holds_alternative<OutputStub>(BoundaryStub)) {
          const OutputStub &BoundaryOutputStub =
              std::get<OutputStub>(BoundaryStub);

          if (FirstOutputStub.Name == BoundaryOutputStub.Name) {
            Avoid::ConnRef *NewRef;

            NewRef = new Avoid::ConnRef(ConstructedRouter, BoxStubPair.second,
                                        BoundaryBoxStubPair.second);
          } else {
